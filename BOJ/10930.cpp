...
string sha256(string msg)
{
    ...
    unsigned int a[8], hv[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
    ...
    int len = msg.length(), blockcnt = (len-1)/56+1;
    for(int i=0;i<blockcnt;i++)
    {
        for(int j=0;j<56;j++)
            tmpblock[j] = (i*56+j<len?(unsigned char)(msg[i*56+j]):(i*56+j>len?0:128));
        tmpblockleninfo = (i!=blockcnt-1?56:(len%56))*8;
        for(int j=0;j<8;j++, tmpblockleninfo/=256)
            tmpblock[63-j] = tmpblockleninfo%256;
        for(int j=0;j<16;j++)
            for(int k=0;k<4;k++)
